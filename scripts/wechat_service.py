...
def delete_wechat_account(account_id):
    ...
    # 删除关联的文章
    db.session.query(WechatArticleList).filter(
        WechatArticleList.wechat_list_id == account_id
    ).delete()
...
@retry(
    ...
    wait=wait_fixed(2),
    retry=retry_if_not_exception_type(ApiAuthError),
    reraise=True,
)
def get_wechat_articles_from_api(wechat_account):
    ...
    try:
        ...
        if ret is not None and ret != 0:
            ...
            if ret in (-3, 200003) or (
                isinstance(err_msg, str)
                and (
                    "invalid session" in err_msg.lower()
                    or "cookie" in err_msg.lower()
...
def parse_and_save_articles(api_response, wechat_account):
    ...
    for item in publish_list:
        try:
            ...
            logger.error(
                f"解析 publish_info 失败: {e}. problematic string: {item.get('publish_info')}"
            )
            continue

        for article_data in appmsgex_list:
            existing_article = (
                db.session.query(WechatArticleList)
                .filter_by(wechat_list_id=wechat_account.id, article_id=article_data.get("aid"))
                .first()
            )
    ...
    with app_context:
        try:
            ...
            collection_tasks[account_id] = {
                "status": "finished",
                "message": f"采集完成，共新增 {total_saved} 篇文章。",
            }
...
def _download_article_content(article, download_dir):
    ...
    try:
        ...
        if head:
            ...
            logger.warning(
                f"文章 '{article.article_title}' 缺少 <head> 标签，无法插入charset声明。"
            )
        ...
        for i, img in enumerate(img_tags):
            ...
            # 处理主图片 (src/data-src)
            if img_url:
                local_img_path = _download_and_replace_image(
                    img_url, i, article, download_dir, assets_dir
                )
                if local_img_path:
            ...
            if srcset:
                ...
                for part in srcset.split(","):
                    ...
                    local_path = _download_and_replace_image(
                        url_part,
...
def _download_articles_worker(task_id, article_ids, app_context):
    ...
    with app_context:
        ...
        def _convert_and_upsert_document(html_path: Path, account_name: str, source_url: str):
            ...
            try:
                ...
                existing = Document.query.filter(
                    Document.file_path.ilike(meta["file_path"])
                ).first()
                # 若存在且mtime一致且status=completed，直接跳过
                if (
                    existing
                    and existing.file_modified_time == meta["file_modified_time"]
                    ...
                    if existing:
                        ...
                        db.session.add(
                            Document(
                                ...
                                file_created_at=meta["file_created_at"],
        ...
        download_tasks[task_id]["message"] = (
            f"下载完成！成功 {completed_count} 篇，失败 {len(article_ids) - completed_count} 篇。开始后台入库处理..."
        )
...
def start_download_task(article_ids, app):
    ...
    thread = Thread(
        target=_download_articles_worker, args=(task_id, article_ids, app.app_context())
    )
    thread.daemon = True
    thread.start()
    return task_id

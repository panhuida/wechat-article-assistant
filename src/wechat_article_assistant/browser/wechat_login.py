...
class WechatLogin:
    ...
    def get_qr_code_url(self) -> Optional[str]:
        ...
        try:
            ...
            try:
                # 尝试等待常见的二维码容器选择器
                page.wait_for_selector(".qrcode, .login_qrcode, #login_qrcode, .qrcode_login", timeout=15000)
            ...
            if not qr_url:
                ...
                for img in all_imgs:
                    ...
                    if src:
                        ...
                        # 通常二维码图片URL包含特定关键词
                        if any(keyword in src.lower() for keyword in ['qrcode', 'qr', 'login', 'scan']):
                            qr_url = src
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    try:
                        qr_element = page.query_selector("img[src*='qrcode'], img[src*='scanloginqrcode']")
                        if not qr_element:
